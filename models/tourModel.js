...
const tourSchema = new mongoose.Schema(
  {
    ...
    ratingsAverage: {
      ...
      set: (val) => Math.round(val * 10) / 10, // 4.6666 * 10 => rount(46.666) => 47 / 10 = 4.7
    },
    ratingsQuantity: {
      type: Number,
      default: 0,
    ...
    startLocation: {
      // GeoJSON is used by mongodb for geospatial data
      type: {
        type: String,
        default: 'Point',
        enum: ['Point'],
    ...
    locations: [
      {
        // GeoJSON is used by mongodb for geospatial datas
        type: {
          type: String,
          default: 'Point',
          enum: ['Point'],
...
//   this.guides = await Promise.all(guides Promises);
//   next();
// });
